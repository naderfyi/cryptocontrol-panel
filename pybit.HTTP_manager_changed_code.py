...
## File name as saved in the server - _http_manager.py
...
class _HTTPManager:
    ...
    def _submit_request_new(self, method=None, path=None, query=None, auth=False):
        """
        Submits the request to the API.
        ...
        # Add agentSource (spot API's Referer).
        if self.referral_id and method == "POST" and path.endswith("/spot/v3/private/order"):
            query["agentSource"] = self.referral_id
    ...
    def _submit_request_cancel(self, method=None, path=None, query=None, auth=False):
        """
        Submits the request to the API.
